...
process_list = [[0,0,14],[1,8,6],[2,9,4]]
k= 0
seconds_passed = 0
starttime= time.time()
active_programs= []
shortest_process= []
print("This is a small program to show how  shortest time to completion firts works given the assumptions that there is no I/O and the execution times are known.")
while True:
    if len(process_list) == 0:
        #all processes are done
        print('\n')
        print("All processes are done, good job CPU!")
        print("It took the STCF algorithm " +str(seconds_passed) + " seconds.")
        break
